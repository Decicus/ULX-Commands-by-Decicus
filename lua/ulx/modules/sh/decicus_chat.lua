...
function NotifyPMuteJoin( ply )

	if ply:GetPData( "ULX_PermMute" ) == "true" then
	
		for _, mods in ipairs( player.GetAll() ) do
		
			if ucl.query( mods, "ulx pmute" ) then
			
				ULib.tsayError( mods, ply:Nick() .. " has joined the server and is permmuted." )
...
hook.Add( "PlayerInitialSpawn", "NotifyPMuteJoin", NotifyPMuteJoin )

function NotifyPMuteLeave( ply )
	...
	if ply:GetPData( "ULX_PermMute" ) == "true" then
		...
		for _, mods in ipairs( player.GetAll() ) do
			...
			if ucl.query( mods, "ulx pmute" ) then
				...
				ULib.tsayError( mods, ply:Nick() .. " has left the server and is permmuted." )
...
hook.Add( "PlayerDisconnected", "NotifyPMuteLeave", NotifyPMuteLeave )